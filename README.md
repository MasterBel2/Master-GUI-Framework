...
`MasterFramework 44` is stored under `WG["MasterFramework 44"]`.
...
Some brief notes on the internal code of MasterFramework (i.e. within the `MasterFramework 44/` folder):

- The global environment (also the `framework` entry in the global environment) refers to the framework itself, that is registered in `WG["MasterFramework 44"]`. 
